...
nativeObject = YAML.load("database.yml", (database) => {
    ...
    app.use(
        ...
        (req, res, next) => {
            ...
            if (!card) {
                res.status(500).send({
                    error: `No card is available for user ${owner}`,
                });
            }
            req.card = card;
            next();
    ...
    app.get("/card/charge/:owner", (req, res) => {
        const { card, cart } = req;
        const { owner } = req.params;
        const country = database.users.find((user) => user.id === owner)
            .country;
    ...
    app.get("/items/:ids", (req, res) => {
        ...
        if (items.includes(undefined)) {
            res.status(500).json({
                error: "A specified ID had no matching item",
            });
        } else {
    ...
    app.get("/tax/:symbol", (req, res) => {
        ...
        const taxRate = database.taxRates.find(
